...
import { makeKeyboard, isQQBot } from '../../lib/buttons.js'
  ...
  if (name.endsWith('视频')) {
    ...
    if ((apiData.videoDirs || []).includes(dir)) {
      ...
      const keyboard = makeKeyboard(e, [[
        { text: '🔄 再来一个', cmd: `#${dir}视频`, style: 1 },
        { text: '🎬 视频菜单', cmd: '#视频菜单', style: 0 }
      ]])
      // 视频走富媒体单独发，keyboard 要挂在 markdown 上，故补一句文字避免空内容报错
      await e.reply([segment.text(`🎬 ${dir}`), segment.video(videoUrl), keyboard].filter(Boolean))
      return true
    }
  }
 ...
 * 需经全局 fileToUrl 处理器上传成公网 https URL（该处理器已独立成本地插件
 * karin-plugin-filetourl，不随本插件发布；详见 README「QQBot 适配说明」）。
...
export const showExpressionHelp = karin.command(/^#?表情包(帮助|菜单)$/, async (e) => {
  try {
    ...
    const buttons = buildDirButtons(e, expressionList, name => `#${name}`)
...
export const showGirlHelp = karin.command(/^#?小姐姐(帮助|菜单)$/, async (e) => {
  try {
    ...
    // 点击按钮直接发送目录名，触发 dynamicMedia 出图
    const buttons = buildDirButtons(e, girlList, name => `#${name}`)
...
export const showVideoHelp = karin.command(/^#?视频(帮助|菜单)$/, async (e) => {
  try {
    ...
    const buttons = buildDirButtons(e, allVideoDirs, name => `#${name}视频`)
