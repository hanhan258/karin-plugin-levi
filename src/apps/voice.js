...
const voiceKb = () => makeKeyboard([['#唱鸭', '#坤坤语音', '#网易云', '#绿茶', '#骂我']])

export const sjcy = karin.command('^#?(唱鸭|随机唱鸭)$', async (e) => {
  await e.reply([segment.text('🎵 随机唱鸭'), segment.record('http://api.yujn.cn/api/changya.php?type=mp3'), voiceKb()])
  return true
}, { name: '随机唱鸭' })

export const sjkk = karin.command('^#?(坤坤语音|随机坤坤)$', async (e) => {
  await e.reply([segment.text('🎤 随机坤坤'), segment.record('http://api.yujn.cn/api/sjkunkun.php?'), voiceKb()])
  return true
}, { name: '随机坤坤' })
...
export const sjwyy = karin.command('^#?(网易云|随机网易云)$', async (e) => {
  ...
  const fnc = async () => {
    ...
    if (result.id) {
      ...
      await e.reply([segment.text('🎶 随机网易云'), segment.record(result.url), voiceKb()])
    } else {
      retryCount++
...
export const maren = karin.command('^#?骂我$', async (e) => {
  await e.reply([segment.text('🗯 骂我'), segment.record('http://api.yujn.cn/api/maren.php?'), voiceKb()])
  return true
}, { name: '骂我' })

export const lvcha = karin.command('^#?(绿茶|随机绿茶)$', async (e) => {
  await e.reply([segment.text('🍵 随机绿茶'), segment.record('https://api.yujn.cn/api/lvcha.php?'), voiceKb()])
  return true
}, { name: '随机绿茶' })
